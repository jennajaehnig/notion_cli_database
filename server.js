...
async function readMsgs(response, sender){
    ...
    if(count == 0) {
        ...
        for (const key in response.results) {
            const sender = response.results[key].properties.Sender.rich_text[0].plain_text;
...
async function deleteMsgs(response){
    ...
    if (count == 0) {
        ...
        for (const key in response.results) {
            ...
            const msg =
                response.results[key].properties.Message.title[0].plain_text;
            console.log(`Message ${key}:\n\tfrom: ${sender}\n\t${msg}\n`);
    ...
    const msgs_to_delete = prompt("Which message would you like to delete? (Type \"all\" or the message ID): ");
